...
router.post(`/applications`, (req, res) => {
    if (!req.body) {
        res.status(400).send(`Can't create an empty application, idiot.`)
    } else {
        ...
        db.Application.create({
            ...
            err ? res.send(`Error creating application: ${err.message}`) : res.status(200).send(`Success!`)
        });
    }
});
...
router.get(`/applications`, (req, res) => {
    ...
    db.Application.findAll({
        ...
        err ? res.send(`Error finding applications: ${err.message}`) : res.status(200).send(`Success!`)
    });
});
...
router.put(`/applications/:id`, (req, res) => {
    if (!req.params.id) {
        ...
        res.status(400).send(`Why are you sending me an empty form?`)
    } else {
        const tokenData = authenticateMe(req)
        ...
        if (!tokenData) {
            ...
            db.Application.update({
                ...
                if (data) {
                    ...
                    res.send(`No entries updated.`)
                }
            }).catch(err => {
                err ? res.send(`Error updating application: ${err.message}`) : res.status(200).send(`Success!`)
            });
        }
    }
});
        ...
        if (!tokenData) {
            ...
            db.Application.destroy({
                ...
                if(data) {
                    ...
                    res.send(`No applications deleted.`)
                }
            }).catch(err => {
                err ? res.send(`Error deleting application: ${err.message}`) : res.status(200).send(`Success!`)
            });
        }
    }
});
